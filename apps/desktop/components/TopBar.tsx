...
import { Search01Icon, Notification03Icon, Logout01Icon } from "hugeicons-react";
import { useSPALStore } from "@spal/core/store";
import { getGreeting } from "@spal/core/lib/utils/dates";
...
export function TopBar() {
  ...
  const { user, activeBusiness, logout } = useSPALStore();
  ...
  return (
    <header className="flex items-center justify-between h-20 px-8 shrink-0" style={{ borderBottom: "1px solid rgba(15,23,42,0.06)" }}>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <button aria-label="Search" className="w-12 h-12 rounded-full bg-white flex items-center justify-center hover:opacity-80 transition-opacity" style={CIRCLE}>
          <Search01Icon size={19} color="#0F172A" />
        </button>
        <button onClick={() => setUnread(0)} aria-label={unread > 0 ? `Notifications, ${unread} unread` : "Notifications"} className="w-12 h-12 rounded-full bg-white flex items-center justify-center relative hover:opacity-80 transition-opacity" style={CIRCLE}>
        ...
        <button onClick={signOut} aria-label="Sign out" className="w-12 h-12 rounded-full bg-white flex items-center justify-center hover:opacity-80 transition-opacity" style={CIRCLE}>
          <Logout01Icon size={19} color="#0F172A" />
        </button>
      </div>
    </header>
  );
