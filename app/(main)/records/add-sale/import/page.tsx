...
import { ArrowLeft01Icon, Upload01Icon, ArrowRight01Icon, Restaurant01Icon, Csv01Icon, File01Icon } from "hugeicons-react";
...
export default function ImportEntryPage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("spal_recent_imports");
  ...
  async function handleFile(file: File | undefined | null) {
    ...
    setRecentFiles((prev) => {
      ...
      try { localStorage.setItem("spal_recent_imports", JSON.stringify(updated)); } catch { /* ignore */ }
      return updated;
    });
      ...
      <div className="px-5">
        ...
        >
          <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: "#E8F5E9" }}>
            <Upload01Icon size={22} color="#22C55E" />
          </div>
        ...
        <AnimatePresence>
          {!parsing && records.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4">
              <div className="rounded-2xl px-4 py-3.5 mb-3" style={{ background: "#E8F5E9" }}>
                <p className="text-[12.5px] font-medium" style={{ fontFamily, color: "#2E7D32" }}>
                ...
                {records.map((r, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="inline-block mt-0.5 text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ background: "#F0FDF4", color: "#16A34A" }}>
                        {r.category ?? "Sales"}
                      </span>
                    </div>
                    <p className="text-[14px] font-bold flex-shrink-0" style={{ fontFamily, color: "#22C55E" }}>
            ...
            <div className="space-y-2">
              {recentFiles.map((f, i) => (
                <div key={i} className="bg-white rounded-2xl px-4 py-3.5 flex items-center gap-3" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  {f.name.endsWith(".csv") ? <Csv01Icon size={20} color="#2563EB" className="flex-shrink-0" /> : <File01Icon size={20} color="#6B7280" className="flex-shrink-0" />}
                  <div className="flex-1 min-w-0">
      ...
      <AnimatePresence>
        {records.length > 0 && !parsing && (
          ...
          >
            ...
            >
              {saving ? "Importing..." : `Import ${records.length} records`}
            </button>
          </motion.div>
        )}
      </AnimatePresence>
